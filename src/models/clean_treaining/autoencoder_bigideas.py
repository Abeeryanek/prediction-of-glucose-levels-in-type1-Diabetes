...
- Grid: latent_size [16,32,64] x lr [1e-3, 5e-4] (§2.3, §8A.5)
...
# §2.3, §8A.5 — autoencoder grid: latent_size x lr
DL_PARAM_GRID = {"latent_size": [16, 32, 64], "lr": [1e-3, 5e-4]}

# exact plain food column names after fix_food_features.py
...
# ============================================================================
# 6. MODEL DEFINITION — Seq2Seq Autoencoder (§2.3)
# ============================================================================
...
class GlucoseSeq2Seq(nn.Module):
    """
    Encoder LSTM -> bottleneck Linear -> decoder head.
    Tight bottleneck forces encoder to retain only the dominant glucose
    trend, reducing overfitting on small per-patient datasets
    (Srivastava et al. 2015, §2.3).
    latent_size is auto-set to hidden_dim // 2 per §8A.5.
    """
    def __init__(self, input_dim, latent_size=16, hidden_dim=64,
    ...
    def forward(self, x):
        ...
        return self.fc_out(decoded)                    # (batch, 1)
    ...
    **model_kwargs,
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            # deep copy — clone each tensor independently
...
def grid_search_dl(x_train, y_train, sample_weights, input_dim, param_grid):
    ...
    for combo in combos:
        ...
        _, val_loss, _ = train_autoencoder(
    ...
    for fold_i, (df_tr, df_te) in enumerate(wf_folds):
        for subset, features in FEATURE_SETS.items():
            for h_label in HORIZONS:
                ...
                model, val_loss, epochs_trained = train_autoencoder(
...
print("\nSeq2Seq Autoencoder pipeline complete (walk-forward + ablation).")