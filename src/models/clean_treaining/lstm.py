"""
lstm.py — LSTM, Window Size x Horizon Walk-Forward Pipeline
====================================================================
Aligned to FINAL_EXPERIMENTS_PLAN.md:

- Clinically-weighted MSELoss (hypo/hyper sample weights)
- Grid: hidden_dim [32,64,128] x lr [1e-3, 5e-4] (§2.2, §8A.5)
...
- LSTM: bidirectional LSTM → Linear
"""
import glob
import os
import json
...
print(f"\n{'='*70}\nLSTM — WINDOW x HORIZON WALK-FORWARD PIPELINE\n{'='*70}")
...
def create_bar_plot(ablation_results, title, save_path):
    ...
    for ax, horizon in zip(axes, horizons):
        ...
        bars   = ax.barh(y_pos, values.values, height=0.65,
                         edgecolor="black", linewidth=0.6, color="cyan")
    ...
    for fold_i, (df_tr, df_te) in enumerate(wf_folds):
        for subset, features in FEATURE_SETS.items():
            for h_label in HORIZONS:
                ...
                model, val_loss, epochs_trained = train_lstm(
...
print("\nLSTM pipeline complete (walk-forward + ablation).")