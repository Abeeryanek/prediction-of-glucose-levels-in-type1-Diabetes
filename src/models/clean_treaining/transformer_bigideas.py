"""
transformer.py — Transformer, Window Size x Horizon Walk-Forward + LOPO Pipeline
===================================================================================
Self-contained: no external config/utils modules. Everything in one file.
Identical structure to lstm.py / cnn_lstm.py — only the model architecture
differs (Transformer encoder with positional embedding, instead of LSTM).
...
2. ALL LOOKBACK-DEPENDENT FEATURES ARE ANCHORED TO THE CURRENT window_size.
   Physiological variability, activity, and the ablation-only glucose lag/roc 
   are all computed using THE SAME window_size as their single rolling window.
...
Transformer specific:
   - Positional embedding is sized to the current window_size (seq_len), so it
     is rebuilt per window_size (handled automatically since model is fresh each time).
   - hidden_dim doubles as d_model; nhead fixed at 4, all candidate hidden_dim
     values [32, 64, 128] are divisible by 4.
"""
import glob
import os
import json
...
NHEAD = 4             # fixed; d_model (=hidden_dim) must be divisible by this
DL_PARAM_GRID = {
    "hidden_dim": [32, 64, 128],   # doubles as d_model, all divisible by NHEAD=4
    "lr": [1e-3, 5e-4],
}
GRID_SEARCH_EPOCHS = 30      # reduced-epoch grid search (documented shortcut, see plan §2.6)
GRID_SEARCH_PATIENCE = 5

# LOPO minimum row/sequence thresholds
LOPO_MIN_TRAIN_ROWS = 200
LOPO_MIN_TEST_ROWS = 50

print(f"\n{'='*70}\nTRANSFORMER — WINDOW x HORIZON WALK-FORWARD + LOPO PIPELINE\n{'='*70}")
...
class GlucoseTransformer(nn.Module):
    """Transformer encoder with multi-head self-attention over the look-back
    window, followed by a linear head on the last time step's representation.
    `hidden_dim` doubles as d_model and must be divisible by `nhead`."""

    def __init__(self, input_dim, hidden_dim=64, nhead=NHEAD, num_layers=2,
                 dropout=0.3, seq_len=12):
        super().__init__()
...
# 7. TRAINING LOOP
# ============================================================================
def train_weighted_transformer(x_train, y_train, sample_weights, input_dim, seq_len,
    ...
    model = GlucoseTransformer(input_dim=input_dim, seq_len=seq_len, **model_kwargs)
...
# 8. GRID SEARCH
# ============================================================================
def grid_search_dl(x_train, y_train, sample_weights, input_dim, seq_len, param_grid):
    ...
    for combo in combos:
        ...
        _, val_loss, _ = train_weighted_transformer(
            x_train, y_train, sample_weights, input_dim=input_dim, seq_len=seq_len,
...
def run_lopo_dl(df_w, features, target_col, seq_length, best_params, patient_col="Patient_ID"):
    ...
    for test_pid in patients:
        ...
        model, _, epochs_trained = train_weighted_transformer(
            X_train_3d, y_train, weights, input_dim=len(features), seq_len=seq_length,
...
for window_label, window_size in WINDOW_SIZES.items():
    ...
    if gs_key in grid_cache:
        ...
        best_params, best_score = grid_search_dl(
            ...
            input_dim=len(all_features), seq_len=SEQ_LENGTH, param_grid=DL_PARAM_GRID,
        )
    ...
    for fold_i, (df_tr, df_te) in enumerate(wf_folds):
        for subset, features in FEATURE_SETS.items():
            for h_label in HORIZONS:
                ...
                model, val_loss, epochs_trained = train_weighted_transformer(
                    seq["X_train"], seq["y_train"], weights,
                    input_dim=len(features), seq_len=SEQ_LENGTH,
                    hidden_dim=best_params["hidden_dim"], lr=best_params["lr"],
                )
                model.eval()
                with torch.no_grad():
    ...
    for combo_name, combo_features in feature_groups.items():
        ...
        for h_label in HORIZONS:
            ...
            model, _, _ = train_weighted_transformer(
                seq["X_train"], seq["y_train"], weights,
                input_dim=len(combo_features), seq_len=SEQ_LENGTH,
                hidden_dim=best_params["hidden_dim"], lr=best_params["lr"],
            )
            model.eval()
            with torch.no_grad():
...
print("\nTransformer pipeline complete (walk-forward + ablation + LOPO).")