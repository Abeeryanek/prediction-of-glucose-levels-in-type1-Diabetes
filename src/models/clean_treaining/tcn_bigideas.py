"""
tcn.py — Temporal Convolutional Network, Window Size x Horizon Walk-Forward + LOPO Pipeline
=================================================================================================
Self-contained: no external config/utils modules. Everything in one file.
Mirrors random_forest.py / gradient_boost.py / cnn_lstm.py structurally, adapted
for the TCN architecture (dilated causal convolutions instead of recurrence).
...
4. LOPO (Leave-One-Patient-Out), nested inside the window_size loop, reusing
   that window's cached grid-search-best num_filters/lr. 3D sequences are
   built PER PATIENT (create_3d_sequences respects patient boundaries via the
   patient_ids check), then pooled — no sequence can span two patients.
...
TCN-specific notes (per plan §2.4):
    - kernel_size fixed at 3
    - dilations = [1, 2, 4, 8] (4 residual blocks, receptive field grows with window)
    - num_filters swept in grid search
"""
import glob
import os
import json
...
MODEL_NAME = "tcn"
WINDOW_SIZES = {"1h": 12}   # lookback, in 5-min steps
...
print(f"\n{'='*70}\nTCN — WINDOW x HORIZON WALK-FORWARD + LOPO PIPELINE\n{'='*70}")
...
class GlucoseTCN(nn.Module):
    def __init__(self, input_dim, num_filters=32, kernel_size=KERNEL_SIZE, dilations=None, dropout=0.3):
        super().__init__()
        dilations = dilations or DILATIONS
        layers = []
        in_channels = input_dim
        for d in dilations:
            layers.append(TemporalBlock(in_channels, num_filters, kernel_size, dilation=d, dropout=dropout))
...
# 7. TRAINING LOOP
# ============================================================================
def train_weighted_tcn(x_train, y_train, sample_weights, input_dim, lr, epochs=MAX_EPOCHS, patience=PATIENCE, val_ratio=VAL_RATIO, **model_kwargs):
...
def grid_search_dl(x_train, y_train, sample_weights, input_dim, param_grid):
    ...
    for combo in combos:
        ...
        _, val_loss, _ = train_weighted_tcn(
...
def run_lopo_dl(df_w, features, target_col, seq_length, best_params, patient_col="Patient_ID"):
    ...
    for test_pid in patients:
        ...
        model, _, epochs_trained = train_weighted_tcn(
            X_train_3d, y_train, weights, input_dim=len(features),
            num_filters=best_params["num_filters"], lr=best_params["lr"],
        )
        model.eval()
        with torch.no_grad():
...
all_results, all_ablation_results, all_lopo_results = [], [], []
...
for window_label, window_size in WINDOW_SIZES.items():
    ...
    for fold_i, (df_tr, df_te) in enumerate(wf_folds):
        for subset, features in FEATURE_SETS.items():
            for h_label in HORIZONS:
                ...
                model, val_loss, epochs_trained = train_weighted_tcn(
                    seq["X_train"], seq["y_train"], weights,
                    input_dim=len(features), num_filters=best_params["num_filters"], lr=best_params["lr"],
                )
                model.eval()
                with torch.no_grad():
    ...
    for combo_name, combo_features in feature_groups.items():
        ...
        for h_label in HORIZONS:
            ...
            model, _, _ = train_weighted_tcn(
                seq["X_train"], seq["y_train"], weights,
                input_dim=len(combo_features), num_filters=best_params["num_filters"], lr=best_params["lr"],
            )
            model.eval()
            with torch.no_grad():
                preds = model(torch.tensor(seq["X_test"])).numpy().flatten()
...
print("\nTCN pipeline complete (walk-forward + ablation + LOPO).")